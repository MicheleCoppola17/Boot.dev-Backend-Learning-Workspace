...
1. Check if the current node is empty (has no value). If it is, return None. This represents an empty tree or a leaf node where deletion has already occurred.
...
class BSTNode:
    def delete(self, val: Any) -> "BSTNode | None":
        ...
        # val == self.val -> found node to delete
        if not self.right:
            return self.left
        if not self.left:
            return self.right
